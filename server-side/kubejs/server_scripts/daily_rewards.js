...
PlayerEvents.loggedIn(event => {
  // Set player's time online back to 0
...
PlayerEvents.tick(event => {
  // Cancel further checks if reward has already been claimed today
  if (event.player.persistentData.lastReward === date) return

  // If no reward claimed today, add 1 tick to time online
  event.player.persistentData.timeOnline++

  // Give reward after 20 minutes online
  if (event.player.persistentData.timeOnline >= 24000) {
    ...
    // Mark reward as claimed for today's date
